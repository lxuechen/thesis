...
As alluded to earlier, to design an iterative DP algorithm, it suffices to design a single step of the algorithm and rely on privacy composition for an overall privacy guarantee.
...
We recall the \emph{Gaussian mechanism} for vector-valued functions, which will be used to privatize gradient computation.
...
\begin{algorithmic}[1]
  \State {\bfseries Input:}
    Release procedure $\M: \X \to \R^p$ with $\ell_2$ sensitivity $C$, input dataset $X$.
...
The Gaussian mechanism will be used to obtain private estimates of the mean of gradients for DP optimizers.
Below, we prove that the Gaussian mechanism is DP.
