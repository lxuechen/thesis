...
This is seen from the fact that the second term is $O(\epsilon^2 k)$ and thus much smaller in $\epsilon$.\footnote{This can be obtained with the standard approximation of $e^x - 1 \approx x$ for small $x$}
Roughly speaking, this makes the privacy leakage $\widetilde{\epsilon}$ grow almost as $O(\sqrt{k})$.

The formulae for $\widetilde{\epsilon}$ and $\widetilde{\delta}$ in \eqref{eq:strong_composition} are obtained with standard concentration bounds, and the values on the right-hand side of the equations are conservative estimates of the true privacy leakage.
...
We use some of these privacy accounting techniques in this work but direct the reader to the original papers for a thorough treatment.
...
Training large deep learning models with DP requires iterative optimizers that have DP guarantees.
...
\begin{algorithmic}[1]
  ...
  \State $q = \nicefrac{B}{N}$ \Comment{Derive the sampling rate.}
  \State $T = \floor{ \nicefrac{E}{q} }$ \Comment{Derive the total iteration count.}
  \State Calculate the noise multiplier $\sigma$ based on $\epsilon, \delta, q, E$.
  \For{ $t = 1, \dots, T $ }
    \State Draw a batch $B_t$ via Poisson sampling each element with probability $q$
    \For{ $x_i \in B_t$ }
        \State $g_t(x_i) \gets \nabla_{\theta_t} \mathcal{L}(x_i)$ \Comment{Compute per-example gradient.}
        \State $\tilde{g_t}(x_i) \gets g_t(x_i) \cdot \min(1, \nicefrac{C}{ \normtwo{g_t(x_i)} })$ \Comment{Clip per-example gradient.}
    \EndFor
    \State \(\triangleright\) Estimate the average gradient with the sampled Gaussian mechanism.
    \State $z_t \sim \mathcal{N}(0, \sigma^2 C^2 I_p)$
    \State $\bar{g_t} = \frac{1}{B} \bracks{ 
        \sum_{x_i \in B_t} \tilde{g_t}(x_i) + z_t
    }$
    \State \(\triangleright\) Update model weights and optimizer state.
    \State $\theta_{t + 1}, s_{t+1} \gets \texttt{WeightUpdate}(\theta_t, s_t, H)$
    \EndFor
\State \Return $\theta_{T}$ \Comment{Alternatively, return the Polyak average.}
\end{algorithmic}
\end{algorithm}
